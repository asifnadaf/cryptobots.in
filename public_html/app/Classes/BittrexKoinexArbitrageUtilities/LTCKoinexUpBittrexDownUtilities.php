...
class LTCKoinexUpBittrexDownUtilities
{
    ...
    public function findOpportunity()
    {
        ...
        if (count($koinexTickerDataFromDB) > 0) {
            ...
            $bittrexLTCRate = round($bittrexTickerResponse->sell[0]->Rate, 8);
            $bittrexLTCQuantity = round($bittrexTickerResponse->sell[0]->Quantity, 8);

            $koinexLTCRate = $koinexTickerDataFromDB->LTCBid;
            ...
            $arbitrageReturns = round(($LTC_BTC_Koinex_Ratio - $bittrexLTCRate) / $bittrexLTCRate * 100, 2);
            ...
            if ($arbitrageReturns >= $minimumGrossPercentGain) {
                $explanation = [];
                $explanation['firstTransaction'] = ' Sell ' . round($koinexLTCQuantity, 8) . ' units of LTC @ Rs ' . round($koinexLTCRate, 8) . ' per unit to get a total of Rs ' . round($koinexLTCQuantity * $koinexLTCRate, 8) . ' in Koinex';
                $explanation['secondTransaction'] = 'Buy ' . round($bittrexLTCQuantity, 8) . ' units of LTC @ BTC ' . round($bittrexLTCRate, 8) . ' per unit to spend a total of BTC ' . round($bittrexLTCQuantity * $bittrexLTCRate, 8) . ' in Bittrex';
                ...
                $instructions ['LTC_KoinexUp_BittrexDown'] = $explanation;
            }
