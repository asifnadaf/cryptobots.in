...
class BCCBittrexUpKoinexDownUtilities
{
    ...
    public function findOpportunity()
    {
        ...
        if (count($koinexTickerDataFromDB) > 0) {
            ...
            $bittrexBCCRate = round($bittrexTickerResponse->buy[0]->Rate, 8);
            $bittrexBCCQuantity = round($bittrexTickerResponse->buy[0]->Quantity, 8);

            $koinexBCCRate = $koinexTickerDataFromDB->BCCAsk;
            ...
            $arbitrageReturns = round(($bittrexBCCRate - $BCC_BTC_Koinex_Ratio) / $BCC_BTC_Koinex_Ratio * 100, 2);
            ...
            if ($arbitrageReturns >= $minimumGrossPercentGain) {
                $explanation = [];
                $explanation['firstTransaction'] = ' Sell ' . round($bittrexBCCQuantity, 8) . ' units of BCC @ BTC ' . round($bittrexBCCRate, 8) . ' per unit to get a total of BTC ' . round($bittrexBCCQuantity * $bittrexBCCRate, 8) . ' in Bittrex';
                $explanation['secondTransaction'] = 'Buy ' . round($koinexBCCQuantity, 8) . ' units of BCC @ Rs. ' . round($koinexBCCRate, 2) . ' per unit to spend a total of Rs. ' . round($koinexBCCQuantity * $koinexBCCRate, 2) . ' in Koinex';
