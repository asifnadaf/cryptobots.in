...
class ETHBittrexUpKoinexDownUtilities
{
    ...
    public function findOpportunity()
    {
        ...
        if (count($koinexTickerDataFromDB) > 0) {
            ...
            $bittrexETHRate = round($bittrexTickerResponse->buy[0]->Rate, 8);
            $bittrexETHQuantity = round($bittrexTickerResponse->buy[0]->Quantity, 8);

            $koinexETHRate = $koinexTickerDataFromDB->ETHAsk;
            ...
            $arbitrageReturns = round(($bittrexETHRate - $ETH_BTC_Koinex_Ratio) / $ETH_BTC_Koinex_Ratio * 100, 2);
            ...
            if ($arbitrageReturns >= $minimumGrossPercentGain) {
                $explanation = [];
                $explanation['firstTransaction'] = ' Sell ' . round($bittrexETHQuantity, 8) . ' units of ETH @ BTC ' . round($bittrexETHRate, 8) . ' per unit to get a total of BTC ' . round($bittrexETHQuantity * $bittrexETHRate, 8) . ' in Bittrex';
                $explanation['secondTransaction'] = 'Buy ' . round($koinexETHQuantity, 8) . ' units of ETH @ Rs. ' . round($koinexETHRate, 2) . ' per unit to spend a total of Rs. ' . round($koinexETHQuantity * $koinexETHRate, 2) . ' in Koinex';
