...
class BittrexAPIs
{
    ...
    private function call($method, $params = array(), $apiKey = false)
    {
        ...
        $sign = hash_hmac('sha512', $uri, $this->secretKey);
        $ch = curl_init($uri);
...
//                Log::info('BittrexAPIs->call() $answer->success == false $answer: ' . serialize($answer));
                return ($answer->message);
//                throw new \Exception ($answer->message);
            }
            return $answer->result;
        } else {
//            Log::info('BittrexAPIs->call() $answer==null; $result: ' . serialize($result));
            return null;
        }
    ...
    public function callAndGetFullResponse($method, $params = array(), $apiKey = false)
    {
        $uri = $this->baseUrl . $method;
        if ($apiKey == true) {
            $params['apikey'] = $this->apiKey;
     ...
     * @param string $market literal for the market (ex: BTC-LTC)
     * @param string $type "settings", "sell" or "both" to identify the type of orderbook to return
    ...
    /**
     * Place a limit settings order in a specific market.
    ...
    /**
     * Place a settings order in a specific market.
     * Make sure you have the proper permissions set on your API keys for this call to work
     * @param string $market literal for the market (ex: BTC-LTC)
     * @param float $quantity the amount to purchase
     * @return array
    ...
    /**
     * Cancel a settings or sell order
     * @param string $uuid id of sell or settings order
     * @return array
     */
     ...
     * @param string $uuid the uuid of the settings or sell order
     * @return array
     */
